## 一、pandas 简介
1. [pandas 中文文档：](https://www.pypandas.cn/klearn-pandas)
2. [安装参考文档：](https://pypi.org/project/sklearn-pandas/1.5.0/)
3. [Numpy 学习文档：](https://numpy.org/doc/)
4. [matplotlib 学习文档：](https://matplotlib.org/contents.html)
5. 数据集代码示例
  - eg1. 数据集初探
```
...
```
import pandas as pd
import numpy as np # numpy 是 python 中的一个数学库
import matplotlib as plt # 用作展示的一个库
import os
pwd = os.path.dirname(os.path.realpath(__file__))
book = os.path.join(pwd,'book_utf8.csv')
# df = pd.read_csv('book_utf8.csv')
df = pd.read_csv(book) # df 是 pandas 中的一种数据类型 DataFrame，可理解为 Excel 中的一个表格
# 输出全部内容
print(df)
...
# 过滤数据
df['star'] == '力荐' # 这个判断返回是一个布尔值
df [ df['star'] == '力荐' ] # 这个判断拿出 star == 力荐 的行内容
...
## 二、pandas 基本数据类型
1. series
  - 理解为 Excel 中的一列，并且自带索引，索引可修改
  ![series数据结构](series数据结构.png)
  - 创建 series 数据结构代码示例
```
import pandas as pd
import numpy as np
...
```
## 三、pandas 数据导入
1. Pandas 支持大量格式的导入，使用的是 read_*() 的形式，如：
```
import pandas as pd
pd.read_excel(r'1.xlsx')
pd.read_csv(r'/root/file.csv', sep='', nrows=10, encoding='utf-8')
pr.read_sql(sql, conn)
```
  - eg1. 代码示例
```
import pandas as pd
...
```
## 四、pandas 数据预处理
1. [Series 学习文档：](https://pandas.pydata.org/pandas-docs/stable/reference/series.html)
2. 判断数据缺失代码示例
```
import pandas as pd
import numpy as np
...
```
## 五、pandas 数据调整
1. [DataFrame 学习文档：](https://pandas.pydata.org/pandas-docs/stable/reference/frame.html)
2. 数据调整代码示例
```
import pandas as pd
# 行列调整
...
# 多对一替换，把 4，5，8 替换为 1000
df.replace([4,5,8], 1000)

# 多对多替换，4 替换为 400，5 替换为 500，8 替换为 800
df.replace({4:400,5:500,8:800})

# 排序
# 按照指定列降序排列，ascending = False 为倒序
...
```
## 六、pandas 基本操作
1. [Pandas 计算功能操作文档：](https://pandas.pydata.org/docs/user_guide/computation.html#method-summary)
  - eg1. 代码示例
```
import pandas as pd
df = pd.DataFrame({"A":[5,3,None,4], 
...
# df2.groupby('type') 返回的是一个 DataFrame 的可迭代对象
...
# 各类型产品的销售数量和销售总额，以 type 进行分组，并且统计二月份的总和，aggregate 表示聚合
...
```
## 八、pandas 多编拼接
1. [MySQL 数据库多表连接学习文档：](https://dev.mysql.com/doc/refman/8.0/en/join.html)
  - eg1. 数据拼接代码示例
```
import pandas as pd
import numpy as np
...
```
## 九、pandas 输出和绘图
1. [plot 学习文档：](https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.plot.html)
2. [seaborn 学习文档：](http://seaborn.pydata.org/tutorial.html)
3. 代码示例
  - eg1. 示例导出代码示例
```
# 导出为.xlsx文件，需要 pip3 install xlwt
...
```
## 十、jieba 分词与提取关键词
1. [jieba 学习文档：](https://github.com/fxsjy/jieba/blob/master/README.md)
  - eg1. 代码示例
```
import jieba
...
for string in strings:
    result = jieba.cut(string, cut_all=False) # 精确模式，默认模式
    print('Default Mode: ' + '/'.join(list(result)))

for string in strings:
    result = jieba.cut(string, cut_all=True) # 全模式，适用于搜索引擎
    print('Full Mode: ' + '/'.join(list(result)))

result = jieba.cut('钟南山院士接受采访新冠不会二次暴发') # 默认是精确模式
print('/'.join(list(result)))
# "新冠" 没有在词典中，但是被Viterbi（启发式方法）算法识别出来了
...
```
## 十一、snowNLP 情感倾向分析
1. [snowNLP 参考学习地址：](https://github.com/isnowfy/snownlp/blob/master/README.md)
  - eg1. snownlp 代码示例
```
